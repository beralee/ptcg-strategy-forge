...
## 2026-09-19 — SDK 公共语义模型、录像与运行保护

- 增加本人天梯决策录像的认证下载、身份绑定、完整性校验和不可变导入；缺失录像明确报告，不从摘要补造训练数据。
- 新增版本化 128/32 公开语义张量与 Base 同层模型候选域，保留旧 profile 和规则执行语义。
- 增加真实教师数据资格、多正例 BC、公开资源关系特征、NumPy 排序网络、ORT 导出及原生对照工具；不包含任何实战策略、训练样本或模型权重。
- 本地评测冻结 native 输入，核对模型真实调用与配对身份；加入机器级有限排队、进程扫描竞态复核、受监督子进程、内存/磁盘/输出/时长保护与完整写盘。
- 新增学习型研发流程文档与模板，明确 BC、Godot、CABT、设备和生产验收是独立声明。
- 更新来源锁及相关回归测试。此次 GitHub 提交排除策略专属工作区、包、题库修改、实验回执和身份密钥；SDK 签名/认证接口不附带任何真实凭据。

## 2026-09-16 — SDK 0.3.1 无网页开发者 CLI 补全

- 创建工作区可从认证账号自动取得完整作者身份；认证命令支持显式 origin 绑定的临时标准输入/环境变量 API Key，不要求 Windows 凭据库。
- 增加本人远端版本列表、详情、资格等待、精确包下载、暂停/恢复参赛；资格失败、超时和取消可由退出码判断。
- 增加本人签名公钥撤销和当前 API Key 自撤销，受服务端实际能力控制；保留 Cookie CSRF、账号隔离及 API Key 不可派生的权限边界。
- README/Quickstart 改为 CLI 首选路径，新增 `docs/26-CLI-ONLY-DEVELOPMENT.md`；原生 BC、全历史录像和引擎评估继续单列缺口。
- 上次 GitHub CI 的两项历史 `.ptcgbot` 审计因缺少锁定 Python 3.11.13 而失败；CI 显式安装并校验该审计运行时，保留全部测试，不改变活动 SDK 的 Python 3.13 要求。
